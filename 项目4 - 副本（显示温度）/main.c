    ...
    while (1)
    {
        if (flag1s)  //每秒更新一次温度
        {
            ...
            if (res)                    //读取成功时，刷新当前温度显示
            {
               ...
               // str[len++] = '.';             //添加小数点
                decT = (decT*1000) / 16;        //二进制的小数部分转换为3位十进制位,重点
               ...
               // LcdShowStr(0, 0, str);        //显示到液晶屏上
			    ...
			    ShowLedNumber(5,intT/10,0);			   //十位
				ShowLedNumber(4,(intT-(intT/10)*10),1);	 //个位
				ShowLedNumber(3,decT/100%10,0);	//十分位
				ShowLedNumber(2,decT/10%10,0);  //百分位
				ShowLedNumber(1,decT%10,0);		//千分位
				if(intT>30)
				flagBuzzOn = 1; //开启蜂鸣器
				else
				flagBuzzOn = 0; //开启蜂鸣器
