#include <reg52.h>
sbit ADDR0 = P1^0;
sbit ADDR1 = P1^1;
sbit ADDR2 = P1^2;
sbit ADDR3 = P1^3;
sbit ENLED = P1^4;
unsigned char code image[] = { 
                   				   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
                                   0xE7,0xE7,0xE7,0xE7,0xE7,0xE7,0x00,0x00, //T
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
                                   0x3C,0x3C,0xBD,0x81,0xDB,0xDB,0xC3,0xE7, //A
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
								   0x98,0x9D,0xAD,0xAD,0xB5,0xB5,0xB9,0x19, //N
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
								   0x80,0xBC,0xAE,0x8E,0xFE,0xFE,0xBC,0x80, //G
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
								   0xE3,0xEB,0xEF,0xEF,0xEF,0xEF,0xEF,0x01, //J
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
								   0x81,0xE7,0xE7,0xE7,0xE7,0xE7,0xE7,0x81, //I
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
								   0xFF,0x80,0xFC,0xFC,0x80,0xFC,0xFC,0x80, //E
								   0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF                                  
};	//图片的字模表
void main()
{
EA = 1; //使能总中断
ENLED = 0; //使能 U4，选择 LED 点阵
ADDR3 = 0;

TMOD = 0x01; //设置 T0 为模式 1
TH0 = 0xFC; //为 T0 赋初值 0xFC67，定时 1ms
TL0 = 0x67;
ET0 = 1; //使能 T0 中断
TR0 = 1; //启动 T0
while (1);
}
/* 定时器 0 中断服务函数 */
void InterruptTimer0() interrupt 1
{
static unsigned char i = 0; //动态扫描的索引
static unsigned char tmr = 0; //250ms 软件定时器
static unsigned char index = 0; //图片刷新索引
TH0 = 0xFC; //重新加载初值
TL0 = 0x67;
//以下代码完成 LED 点阵动态扫描刷新
P0 = 0xFF; //显示消隐
switch (i)
{
case 0: ADDR2=1; ADDR1=1; ADDR0=1; i++; P0=image[index+0]; break;
case 1: ADDR2=1; ADDR1=1; ADDR0=0; i++; P0=image[index+1]; break;
case 2: ADDR2=1; ADDR1=0; ADDR0=1; i++; P0=image[index+2]; break;
case 3: ADDR2=1; ADDR1=0; ADDR0=0; i++; P0=image[index+3]; break;
case 4: ADDR2=0; ADDR1=1; ADDR0=1; i++; P0=image[index+4]; break;
case 5: ADDR2=0; ADDR1=1; ADDR0=0; i++; P0=image[index+5]; break;
case 6: ADDR2=0; ADDR1=0; ADDR0=1; i++; P0=image[index+6]; break;
case 7: ADDR2=0; ADDR1=0; ADDR0=0; i=0; P0=image[index+7]; break;
default: break;
}
//以下代码完成每 250ms 改变一帧图像
tmr++;
if (tmr >= 250) //达到 250ms 时改变一次图片索引
{
tmr = 0;
index++;
if (index >= 112) //图片索引达到 112 后归零
{
index = 0;
}
}
}