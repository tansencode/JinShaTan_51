...
#define PSW_MAX_LEN    6    //用宏定义出密码最大长度（最大不超过16）
...
0x89, 0x89, 0xFF, 0x7F, 0xBF, 0xCF, 0xDF, 0xEF
};
...
static unsigned int n=5;   //数码管显示位索引
void ledon();
extern void KeyScan();
...
void KeyAction(unsigned char keycode)
{
    if  ((keycode>='0') && (keycode<='9'))  //输入字符
    {
		...
		num = (keycode-0x30); //整体十进制左移，新数字进入个位
	switch(n)
		  {
		  case 5:
	        ShowLedNumber(5,num,0); //运算结果显示到数码管 
			n--;
			num=0;
			break;
			case 4:
			ShowLedNumber(4,num,0); //运算结果显示到数码管 
			n--;
			num=0;
			break;
			case 3:
			ShowLedNumber(3,num,0); //运算结果显示到数码管 
			n--;
			num=0;
			break;
			case 2:
			ShowLedNumber(2,num,0); //运算结果显示到数码管 
			n--;
			num=0;
			break;
			case 1:
			ShowLedNumber(1,num,0); //运算结果显示到数码管 
			n--;
			break;
			case 0:
			ShowLedNumber(0,num,0); //运算结果显示到数码管 
			n=5;
			num=0;
			break;
			default:
